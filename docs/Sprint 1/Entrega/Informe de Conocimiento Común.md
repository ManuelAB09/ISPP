# Informe de Conocimiento Común

## Sprint S1

### Grupo 9 – Turno de tarde

![Logo App](../../images/logoapp.jpeg)

---

**Proyecto:** MeerKatters  
**Documento:** Entrega  
**Sprint:** Sprint S1  
**Semana:** Semana 2  
**Estado:** Aprobado
**Fecha:** 03/03/2026  
**Autor(es):** Alejandro Soult Toscano

---

## Índice

1. [Enlace a la Base de Conocimiento compartida](#1-enlace-a-la-base-de-conocimiento-compartida)
2. [Contenido específico aportado](#2-contenido-específico-aportado)
3. [Fortalezas, Debilidades y Acciones Tomadas en el grupo](#3-fortalezas-debilidades-y-acciones-tomadas-en-el-grupo)

---

## 1. Enlace a la Base de Conocimiento compartida

Toda la información presente se encuentra en un [documento excel compartido](https://uses0-my.sharepoint.com/:x:/g/personal/alereyper_alum_us_es/IQCT5FMXpD2HTYXddK6Er3KwATUqaO9z2soT8Vu6Uwl-nxs?e=20UbB6) entre los líderes/representantes de cada grupo, de forma que solo ellos pueden editarlo.

---

## 2. Contenido específico aportado

Nuestro equipo ha contribuido con el feedback específico recibido el 05/02/2026 y el 12/02/2026 y acciones tomadas. Además, se ha contribuido al feedback general de ambas sesiones de evaluación. Todo se encuentra en las hojas llamadas "Grupo 9 - MeerKatters" y "General Turno de Tarde"

---

## 3. Fortalezas, Debilidades y Acciones Tomadas en el grupo

### Semana 1: Sesión de presentación del 26/02/2026

**Fortalezas identificadas:**

- Los tres miembros del equipo realizaron una presentación sólida y bien ejecutada.
- Killer Opener muy bien planteado, breve, claro y efectivo; momento especialmente destacable.
- Buena ejecución estética de la presentación, con una línea visual cuidada.
- Mejora notable en la puesta en escena y en la seguridad al presentar.
- Parte económica presentada de forma visualmente atractiva.
  
**Debilidades identificadas:**

- El Killer Opener podría reutilizarse estratégicamente a lo largo de la presentación para reforzar el mensaje.
- Falta de fluidez entre secciones; existen saltos bruscos entre apartados.
- Exceso de tiempo dedicado al análisis de competidores; se recomienda centrarse únicamente en los 2–3 más relevantes.
- Riesgo elevado en realizar una demo en directo (problemas de carga, red inestable, mala percepción si falla); valorar el uso de vídeo pregrabado.
- El diagrama de Gantt presenta demasiados colores y poca claridad visual; debería simplificarse y desglosarse para mayor detalle.
- Afirmaciones como “rentable desde el minuto 1” carecen de base sólida en esta fase del proyecto.
- Falta mayor detalle sobre el origen del presupuesto y proyección económica futura.
- Insuficiente justificación de cómo encaja el presupuesto con los usuarios piloto actuales.
- Se podría representar el punto de equilibrio mediante gráficas para mayor claridad.
- Escasa profundización en problemas, riesgos y planes de contingencia.
- El soporte visual no siempre acompaña adecuadamente la explicación oral.
  
**Acciones tomadas:**

- Se ha reestructurado las diapositivas de forma que siempre haya fluidez y coherencia narrativa entre las secciones.
- Se ha eliminado los competidores menos relevantes de la presentación.
- Se ha considerado la idea de grabar en vídeo la funcionalidad de la demo.
- Se ha corregido el diagrama de gantt, cambiando colores y detallándolo más.
- Se ha desglosado el presupuesto en la presentación.
- Se ha considerado hacer gráficas para señalizar el punto de equilibrio entre gastos y ganancias.
- Se ha planeado introducir los elementos visual de forma que se adapten al discurso, y no al revés.
