...
1. **Configurar el servicio de Windows**
   - Abre `services.msc` (Win + R y escribe `services.msc`).
   - Busca `PostgreSQL <tu versión>` en la lista.
   - Haz clic derecho → **Propiedades** y cambia el **Tipo de inicio** a `Automático`.
   - Pulsa **Iniciar** si el estado está detenido.
...
2. **Usar el script de arranque del repositorio**
   ...
   corriendo, lo inicia si es necesario y luego lanza `./mvnw spring-boot:run`.
   ```powershell
   # desde la carpeta del proyecto (no desde 'backend')
   cd C:\Users\Manukudasai\Desktop\Universidad\ISPP\ISPP
   .\start-backend.ps1
   ```
   Si lo ejecutas desde `backend\` debes prefijar `..\start-backend.ps1` o copiar el script allí.
