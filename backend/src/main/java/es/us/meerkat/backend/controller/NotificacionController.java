...
/**
 * Controlador REST para obtener las notificaciones del usuario autenticado.
...
public class NotificacionController {
    @Operation(summary = "Marcar notificación como leída", description = "Marca una notificación como leída por su ID.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
    ...
    @GetMapping
    @Operation(summary = "Obtener notificaciones del usuario", description = "Devuelve todas las notificaciones del usuario, ordenadas por fecha descendente.", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Notificaciones obtenidas correctamente"),
            @ApiResponse(responseCode = "401", description = "Usuario no autenticado")
    })
    public ResponseEntity<List<Notificacion>> getMyNotifications(@AuthenticationPrincipal Usuario usuario) {
        if (usuario == null) {
