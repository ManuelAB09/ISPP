...
public class CommunityService {
     ...
     */
    public Comunidad createCommunity(Long userId, String nombre, String descripcion, TipoGrupo tipoGrupo, String imagenUrl) {
        ...
        if (usuarioRepository.findById(userId).isPresent()) {
            long freeCommunities = comunidadRepository.countByCreadorIdAndTipoPlan(userId, TipoPlanComunidad.FREE);
            if (freeCommunities >= MAX_FREE_COMMUNITIES) {
                throw new IllegalArgumentException("Se ha alcanzado el límite de 3 comunidades gratuitas. Actualiza a Premium para crear más.");
            }
        }
    ...
    public Comunidad getCommunityById(Long communityId, Long userId) {
        ...
        if (comunidad.getTipoGrupo() == TipoGrupo.GRUPO_PRIVADO && userId != null) {
            if (!authorizationService.isMemberOf(userId, communityId)) {
                throw new IllegalArgumentException("No tienes permiso para acceder a esta comunidad privada");
            }
        }
     ...
     */
    public Comunidad updateCommunity(Long userId, Long communityId, String nombre, String descripcion, String imagenUrl) {
    ...
    public int getMaxMembers(Long communityId) {
        return comunidadRepository.findById(communityId)
