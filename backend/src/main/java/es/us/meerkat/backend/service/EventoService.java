    ...
    public Evento editarEvento(
        ...
        if (huboCambios) {
            ...
            if (!Objects.equals(tituloAnterior, eventoActualizado.getTitulo())) {
                cambios.append("Título: '" + tituloAnterior + "' → '" + eventoActualizado.getTitulo() + "'\n");
            }
            ...
            if (!Objects.equals(fechaHoraAnterior, eventoActualizado.getFechaHora())) {
                cambios.append(
                        "Fecha de inicio: '" + fechaHoraAnterior + "' → '" + eventoActualizado.getFechaHora() + "'\n");
            }
            if (!Objects.equals(fechaFinAnterior, eventoActualizado.getFechaFin())) {
                cambios.append(
                        "Fecha de fin: '" + fechaFinAnterior + "' → '" + eventoActualizado.getFechaFin() + "'\n");
            }
            if (!Objects.equals(aforoAnterior, eventoActualizado.getAforo())) {
                cambios.append("Aforo: '" + aforoAnterior + "' → '" + eventoActualizado.getAforo() + "'\n");
            }
            if (!Objects.equals(queLlevarAnterior, eventoActualizado.getQueLlevar())) {
                String antes = (queLlevarAnterior == null || queLlevarAnterior.trim().isEmpty()) ? "vacío"
                        : queLlevarAnterior;
                String despues = (eventoActualizado.getQueLlevar() == null
                        || eventoActualizado.getQueLlevar().trim().isEmpty()) ? "vacío"
            ...
            if (!Objects.equals(esVirtualAnterior, eventoActualizado.getEsVirtual())) {
                cambios.append("Tipo: " + (eventoActualizado.getEsVirtual() ? "Virtual" : "Presencial") + "\n");
            }
            if (!Objects.equals(privadoAnterior, eventoActualizado.getPrivado())) {
                cambios.append("Privacidad: " + (eventoActualizado.getPrivado() ? "Privado" : "Público") + "\n");
            }
            ...
            if (!Objects.equals(visibleMapaAnterior, eventoActualizado.getVisibleMapa())) {
                cambios.append("Visibilidad en mapa: " + (eventoActualizado.getVisibleMapa() ? "Sí" : "No") + "\n");
            }
            ...
            for (Long usuarioId : miembrosIds) {
                if (!usuarioId.equals(eventoActualizado.getCreador().getId())) {
                    ...
                    if (usuario != null) {
                        Notificacion notificacion = Notificacion.builder()
                                .usuario(usuario)
                                .titulo("Evento actualizado en la comunidad " + comunidad.getNombre())
                                .mensaje("El evento '" + eventoActualizado.getTitulo()
                                        + "' ha sido actualizado.\n\nCambios:\n" + resumenCambios)
                                .tipo("EVENTO")
                                .leida(false)
                                .comunidadId(comunidad.getId())
