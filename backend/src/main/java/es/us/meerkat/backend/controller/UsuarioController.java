 ...
 *
 * Implementa los endpoints del tag Usuarios del OpenAPI.
 * Usa {@link AuthenticationPrincipal} para obtener el usuario
 * autenticado directamente del contexto de seguridad.
...
public final class UsuarioController {
    ...
    public ResponseEntity<UserDetailResponse> getMe(
        ...
        return ResponseEntity.ok(
            usuarioService.obtenerPerfilPropio(usuario));
    }
     ...
     * Devuelve 204 sin contenido tras la eliminación.
     *
    ...
    public ResponseEntity<UserDetailResponse> updateVisibility(
        ...
        return ResponseEntity.ok(
            usuarioService.actualizarVisibilidad(usuario, request));
    }
     ...
     * Accesible sin autenticación si el perfil es visible.
     *
     * @param userId ID del usuario cuyo perfil se quiere ver.
     * @return Perfil público del usuario.
     */
    @GetMapping("/{userId}")
    public ResponseEntity<UserPublicResponse> getUserById(
            @PathVariable final Long userId) {
        return ResponseEntity.ok(
            usuarioService.obtenerPerfilPublico(userId));
    }
}
