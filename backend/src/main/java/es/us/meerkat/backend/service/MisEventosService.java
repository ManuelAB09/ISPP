...
/**
 * Servicio para gestionar el listado unificado "Mis Eventos" y las alertas
 * automáticas.
 *
 * <p>
 * Proporciona:
 *
 * <ul>
 * <li>Listado de todos los eventos próximos del usuario con información visual
 * de proximidad.
...
public class MisEventosService {
         ...
         *
         * <p>
         * Incluye todos los eventos de las comunidades del usuario que aún no han
         * comenzado,
        ...
        public List<MisEventosItemResponse> obtenerMisEventos(final Long usuarioId) {
                ...
                final List<Evento> eventos = eventoRepository.findProximosEventosByUsuarioId(usuarioId, ahora);
        ...
        public AlertaEventoResponse marcarAlertaComoLeida(final Long alertaId, final Long usuarioId) {
                final AlertaEvento alerta = alertaEventoRepository
                                .findById(alertaId)
        ...
        /**
         * Genera alertas para todos los usuarios cuyos eventos comiencen en el rango
         * especificado. Este
        ...
        public void generarAlertasParaRango(
                ...
                // rango
                final List<Long> eventosConAlertaExistente = alertaEventoRepository
                                .findEventoIdsWithAlertaInRange(desde, hasta, tipoAlerta);
        ...
        /**
         * Genera alertas individuales para todos los miembros de la comunidad del
         * evento.
         *
        ...
        private void generarAlertasParaEvento(final Evento evento, final TipoAlerta tipoAlerta) {
                ...
                final List<Usuario> miembros = usuarioRepository
                                .findMiembrosByComunidadId(evento.getComunidad().getId());
                ...
                miembros.forEach(
                                usuario -> {
                                        ...
                                        boolean yaExiste = alertaEventoRepository
        ...
        /**
         * Convierte una entidad Evento a MisEventosItemResponse con información de
         * proximidad.
         */
        private MisEventosItemResponse mapToMisEventosItem(
                ...
                final boolean proximaEn24H = minutosHastaEvento >= 0 && minutosHastaEvento <= (HORAS_ALERTA_24H * 60);
                final boolean inminenteEn15Min = minutosHastaEvento >= 0
                                && minutosHastaEvento <= MINUTOS_ALERTA_INMINENTE;

                // Verificar si el usuario confirmó asistencia
                boolean asistenciaConfirmada = asistenciaEventoRepository
                ...
                return MisEventosItemResponse.builder()
                                ...
                                .comunidadNombre(
                                                evento.getComunidad() != null ? evento.getComunidad().getNombre()
                                                                : null)
                                .comunidadId(evento.getComunidad() != null ? evento.getComunidad().getId() : null)
                                .ubicacionNombre(
                                                evento.getUbicacion() != null ? evento.getUbicacion().getNombre()
                                                                : null)
        ...
        private AlertaEventoResponse mapToAlertaResponse(final AlertaEvento alerta) {
                ...
                final TipoEvento tipoEvento = evento.getTipoEvento() != null ? evento.getTipoEvento() : TipoEvento.OTRO;
        ...
        private String buildMensajeAlerta(final Evento evento, final TipoAlerta tipoAlerta) {
                return switch (tipoAlerta) {
                        ...
                        default ->
                                String.format("🔔 Tienes una alerta para el evento \"%s\".", evento.getTitulo());
                };
        }
        ...
        /**
         * Elimina alertas leídas con más de 7 días de antigüedad. Llamado por el
         * scheduler diario de
         * limpieza.
         */
        @Transactional
