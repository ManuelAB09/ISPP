 ...
 *
 * <p>
 * Centraliza el manejo de excepciones y proporciona respuestas HTTP coherentes
 * según el tipo de
 * error. Excluye rutas de Swagger para evitar interferencias con la
...
public class GlobalExceptionHandler {
        ...
        public ResponseEntity<ErrorResponse> handleMethodArgumentNotValid(
                ...
                ex.getBindingResult()
                                .getFieldErrors()
                                .forEach(
                                                error -> fieldErrors.putIfAbsent(
                                                                error.getField(), error.getDefaultMessage()));

                final String validationDetails = fieldErrors.values().stream().collect(Collectors.joining("; "));

                final String message = validationDetails.isBlank()
                                ? "Datos de entrada inválidos"
                                : "Datos de entrada inválidos: " + validationDetails;

                final ErrorResponse errorResponse = new ErrorResponse(HttpStatus.BAD_REQUEST.value(), message,
        ...
        public ResponseEntity<ErrorResponse> handleDataIntegrityViolationException(
                ...
                final String message = "Conflicto de datos al guardar. Revisa que la franja no se solape con otra"
                                + " existente.";
                final ErrorResponse errorResponse = new ErrorResponse(HttpStatus.CONFLICT.value(), message,
                                request.getRequestURI());
                return ResponseEntity.status(HttpStatus.CONFLICT).body(errorResponse);
        }
         ...
         *
         * <p>
         * NOTA: Aunque esté aquí, no afectará a las rutas de Swagger porque
