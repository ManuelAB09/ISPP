...
public class MensajeComunidadLeidoController {
    @Autowired
    private MensajeComunidadLeidoService mensajeComunidadLeidoService;
    @Autowired
    private MensajeComunidadRepository mensajeComunidadRepository;

    @PostMapping("/marcar-leido/{mensajeId}")
    @Operation(summary = "Marcar mensaje de comunidad como leído", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses({
    ...
    @PostMapping("/leidos")
    @Operation(summary = "Obtener ids de mensajes de comunidad leídos de una lista", security = @SecurityRequirement(name = "bearerAuth"))
    public ResponseEntity<List<Long>> obtenerLeidos(@RequestBody Map<String, List<Long>> body,
        ...
        List<Long> idsLeidos = mensajeComunidadLeidoService.obtenerIdsMensajesLeidos(usuario.getId(), mensajeIds);
