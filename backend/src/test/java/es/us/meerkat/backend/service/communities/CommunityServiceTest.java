...
class CommunityServiceTest {
    ...
    void createCommunityShouldCreateFreeCommunityAndAssignAdmin() {
        ...
        when(miembroComunidadRepository.countByUsuarioIdAndRolAndComunidadInstitutionIsNull(userId, RolComunidad.ADMIN)).thenReturn(0L);
    ...
    void createCommunityShouldFailWhenFreeLimitReached() {
        ...
        when(suscripcionService.obtenerMiSuscripcion(userId))
                .thenReturn(Optional.of(Suscripcion.builder().plan(TipoPlan.FREE).build()));
        when(miembroComunidadRepository.countByUsuarioIdAndRolAndComunidadInstitutionIsNull(userId, RolComunidad.ADMIN)).thenReturn(3L);
