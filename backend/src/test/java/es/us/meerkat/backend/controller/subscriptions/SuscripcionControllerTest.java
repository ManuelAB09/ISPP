...
class SuscripcionControllerTest {
    ...
    @Test
    @DisplayName(
            "POST /me/verify-session - Debe devolver BAD_REQUEST si la sesión no está completa")
    ...
    @Test
    @DisplayName(
            "POST /me/verify-session - Debe devolver FORBIDDEN si la sesión pertenece a otro"
                    + " usuario")
    ...
    @Test
    @DisplayName(
            "POST /me/verify-session - Debe devolver INTERNAL_SERVER_ERROR ante error de Stripe")
    ...
    @Test
    @DisplayName(
            "POST /me/confirm-embedded-payment - Debe devolver BAD_REQUEST sin paymentIntentId")
    ...
    @Test
    @DisplayName(
            "POST /me/confirm-embedded-payment - Debe devolver BAD_REQUEST si el pago no está"
                    + " completado")
    ...
    @Test
    @DisplayName(
            "POST /me/confirm-embedded-payment - Debe devolver FORBIDDEN si el intent no pertenece"
                    + " al usuario")
    void testConfirmarPagoEmbebido_UsuarioNoCoincide() throws Exception {
        ...
        when(intent.getMetadata())
                .thenReturn(Map.of("usuarioId", "999", "periodo", "mensual", "plan", "PREMIUM"));
    ...
    @Test
    @DisplayName(
            "POST /me/confirm-embedded-payment - Debe activar suscripción cuando el intent es"
                    + " válido")
    void testConfirmarPagoEmbebido_ActivaSuscripcion() throws Exception {
        ...
        try (MockedStatic<com.stripe.model.PaymentIntent> mockedIntent =
                mockStatic(com.stripe.model.PaymentIntent.class)) {
            mockedIntent
                    .when(() -> com.stripe.model.PaymentIntent.retrieve("pi_ok"))
                    .thenReturn(intent);
